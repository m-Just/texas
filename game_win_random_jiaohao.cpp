...
int get_uplim(double win_rate, int jet, int mybet, int round)/*win_rate is winrate or relwinrate*/
{
	...
	double ans1 = ((double)tmp * win_rate * para) / (1.0 - min(0.99999999999999, win_rate * exp(win_rate - 0.35 - ((8 - not_fold_plnum) * 0.05))  * para));
	int ans;
...
int main(int argc, char* agrv[]){
	...
	//			printf("stage: %d, not_fold_plnum:%d\n", stage, not_fold_plnum);
				...
				if(stage == 1){//before flop
					uplim = get_uplim(winrate, my.jetton, mybet, round) / 2;
				...
				if(stage >= 2){
					...
					/*for(i = 1; i <= done[0].pid; i++){
						...
						if(tmp != -1 && tmp != 0){
							...
							rel_winrate *= (1 + (REL_WINRATE_MULT * (hold_poker - tmp) - pos) * (((double)round + 1.0) / 200.0));
						}
					}*/
					double ret = 1.0;
